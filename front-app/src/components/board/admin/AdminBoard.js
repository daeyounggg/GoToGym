...
const AdminBoard = ({ errors, items }) => {
  ...
  const handleUpdateClick = () => {
    // 실제로 선택된 ID를 사용한 업데이트 로직으로 교체하세요
    console.log('업데이트 중:', selectedIds);
  };

  const handleDeleteClick = () => {
    // 실제로 선택된 ID를 사용한 삭제 로직으로 교체하세요
    console.log('삭제 중:', selectedIds);
  };
                  ...
                  <tr key={index}>
                    <td align="center">
                      <input
                        ...
                        onChange={handleCheckboxChange}
                      />
                      <label htmlFor={`bid_${index}`}></label>
                    </td>
                    <td align="center">{item.bId}</td>
                    <td align="center">{item.bName}</td>
                    <td>
                      <a href={`/admin/board/${item.bId}/update`}>설정수정</a>
                      <a href={`/board/list/${item.bId}`}>미리보기</a>
                      <a href={`/board/write/${item.bId}`}>글쓰기</a>
                    </td>
                  </tr>
                ))}
            </tbody>
